...
Since we have the same region throught the dataset, we will consider dropping Region from the dataframe.
...
1. The median number of family members is higher in a household with male head, than felale head.
...
3. The median value of the number of family members with no electricity is a bit higher than the number of members with exectricity; though the spread seems to be the same.
...
2. around 8% and 16% of the Extended family and Singele family households have no electricity, respectively.
...
1. Around 85% of household with no electricity have male heads.

2. Around 78% of household with electricity have male heads.
...
For our initial analysis, Model 1, a main effects model was considered. Insignificant terms were dropped one by one on the basis of p-value and arrived at Model 2. From Table \ref{tab:anova1}, Model 2 did not seem to fit the data better. So, Model 1 was preferred. 
...
```{r anova4}
anova(model4, model5, test = "Chisq") %>% kable(caption = '\\label{tab:anova3}Compare Model 4 and 5', digits = 2) %>%
  kable_styling(font_size = 10, latex_options = "hold_position")
```
